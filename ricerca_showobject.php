...
<html lang="en">
	<head>
		...
		<script src="http://maps.google.com/maps/api/js?key=AIzaSyCRqpcDNzaBXhSxlMXBPJgks9HrvJzQ6sg" type="text/javascript"></script>
	</head>
...
<?php include 'navbar.html'; ?>
	...
	<div class="container form-style">
		<div class="row">
			...
			<div class="span5">
				<?php
					//Seleziono ID dell'Utente loggato
					$strSQL = "SELECT IDUtente FROM Autenticazione WHERE NomeUtente = '".$_SESSION['username']."'";
					$query_result1 = mysql_query($strSQL);
					$row = mysql_fetch_array($query_result1);
					
					//Controllo presenza Oggetto in preferiti
					$strSQL = "SELECT IDPreferito FROM Preferiti WHERE Oggetto = ".$_GET['objID']." AND Utente = ".$row['IDUtente'].""; 
					$query_result1 = mysql_query($strSQL);
					
					echo"<br><div id=prezzo align=right><h3> $result[Prezzo] &euro; </h3></div>";  
					echo"<div id=nomeUtente align=right><small> Inserito il $result[OData], alle $result[OOra], dall'utente <a id=sendmessage href=# style='text-decoration: none'>$result[NomeUtente]</a> </small></div><br>";
					
					echo"<div align=right> Aggiungi l'oggetto ai preferiti &nbsp;";
					
					//Controllo preferiti
					if(mysql_num_rows($query_result1) == 0){
						echo"<a id=ref href='#'><img id=img src='pics/starW.jpg'></a> <br><br> </div>";
						echo"<input id=flag type=hidden value='0'>";
						}
					else{
						echo"<a id=ref href='#'><img id=img src='pics/starY.jpg'></a> <br><br> </div>";
						echo"<input id=flag type=hidden value='1'>";
						}
						
					echo"<input id=user type=hidden value=$row[IDUtente]>";
					echo"<input id=obj type=hidden value=$_GET[objID]>";
					...
					if($result['YNMap'] == 1){
						if($result['Indirizzo'] != "" || $result['Indirizzo'] != NULL) 
							echo"Indirizzo: <em><span id=address>$result[Indirizzo]</span></em><br> ";
						if(($result['Paese'] != "" || $result['Paese'] != NULL) && ($result['CAP'] != "" || $result['CAP'] != NULL)) {
							$paese = ucwords(strtolower($result['Paese']));
							echo"<em><span id=paese>$paese</span></em>, <em><span id=cap>$result[CAP]</span></em> ";
							}
						}
						if($result['Provincia'] != "" || $result['Provincia'] != NULL)
						echo"<br>Provincia: <em><span id=Prov>$result[Provincia]</span></em>";
						
					if($result['YNTel'] == 1 && ($result['TUtente'] != NULL || $result['TUtente'] != ""))
						echo"<span id=Tel>Numero di telefono: <em>$result[TUtente]</em></span></div>";
		...
		<div class="row" style="padding-left:20px;">
			<div class="span12">
				<?php
					echo"<div align=justify><h4> Descrizione dell'Oggetto: </h4>";
					echo"<div id=descrizione2> ".htmlentities($result['Descrizione'], ENT_QUOTES, 'UTF-8')." </div></div><br><br><br>";
				?>
			</div>
		</div>
		<div class="row" style="padding-left:20px;">
